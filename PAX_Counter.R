...
NHSBoards_tidy <- merge(NHSBoards_tidy,datatest,by.x = "id",by.y = "ROOM_ID")
NHSBoards_tidy$Flooring_Time_Hour <- floor_date(NHSBoards_tidy$Time,"hour")
NHSBoards_tidy_tmp <- NHSBoards_tidy %>% select(long,lat,NUMBER,id,Flooring_Time_Hour,group)#%>% group_by(long,lat,FID,Flooring_Time_Hour,group) %>% summarise(Persons = NUMBER)
ggplot(NHSBoards_tidy_tmp, aes(x = long, y = lat,group = group,fill = NUMBER)) +
